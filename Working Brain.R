...
ui <- fluidPage(
  ...
  tags$script(HTML("
    ...
    function initScene() {
      ...
      // Block wheel to avoid scroll-zoom behavior
    ...
    function makeTextSprite(message) {
      ...
      var material = new THREE.SpriteMaterial({
        map: texture,
        transparent: true,
        depthTest: false,   // always on top, but we hide by camera-side test
        depthWrite: false
      });
...
server <- function(input, output, session) {
  ...
  # Load default brain from app directory (e.g., www/)
  observeEvent(TRUE, {
    app_dir <- getwd()
    glb_files <- list.files(
      app_dir,
      pattern   = "\\.glb$",
      full.names = TRUE,
      recursive  = TRUE
    ...
    if (length(glb_files) == 0) {
      showNotification(
        "No .glb brain model bundled with the app. Please upload one using 'Upload Different Brain Model (optional)'.",
        type = "error"
      )
      return()
    }
    
    # Prefer a file with 'Brain_MRI' in the name if present
    ...
    message("Using bundled brain model: ", default_brain_path)
